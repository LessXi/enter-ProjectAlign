...
export default function PurchaseOrders() {
  const { items, purchaseOrders, currentRole, createPO, updatePOStatus } = useInventoryStore();
  ...
  const handleCreate = (status: POStatus) => {
        ...
        <div className="bg-card rounded-lg shadow-card p-6">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  if (item) setPOPrice(String(item.unitPrice));
                }}
          ...
          <div className="flex justify-end gap-3 mt-6">
            <button onClick={() => setShowForm(false)}
              className="px-4 py-2 border rounded-md text-sm font-medium hover:bg-secondary transition-colors">取消</button>
            <button onClick={() => handleCreate('draft')}
              disabled={!poItemId || !poQty || !poPrice || !poSupplier}
              className="px-4 py-2 border rounded-md text-sm font-medium hover:bg-secondary transition-colors disabled:opacity-50">
              保存草稿
            </button>
            <button onClick={() => handleCreate('pending')}
              disabled={!poItemId || !poQty || !poPrice || !poSupplier}
      ...
      {selectedPO && (
        <div className="bg-card rounded-lg shadow-card p-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            ...
            <div><span className="text-muted-foreground">状态：</span><POBadge status={selectedPO.status} /></div>
                    ...
                    <td className="py-3 px-4 text-center">
                      <div className="flex items-center justify-center gap-1">
                        ...
                        {isBoss && po.status === 'pending' && (
                          <>
                            <button onClick={() => updatePOStatus(po.poNumber, 'approved')} className="p-1.5 hover:bg-success-bg rounded" title="批准">
                              <Check className="w-4 h-4 text-success" />
                            </button>
                            <button onClick={() => updatePOStatus(po.poNumber, 'rejected')} className="p-1.5 hover:bg-destructive-bg rounded" title="驳回">
                              <X className="w-4 h-4 text-destructive" />
                            </button>
                          </>
                        )}
                        {isPurchasing && po.status === 'draft' && (
                          <button onClick={() => updatePOStatus(po.poNumber, 'pending')} className="px-2 py-1 text-xs bg-primary text-primary-foreground rounded hover:bg-primary/90">
                            提交
                          </button>
                        )}
                        {isPurchasing && po.status === 'approved' && (
                          <button onClick={() => updatePOStatus(po.poNumber, 'received')} className="px-2 py-1 text-xs bg-success text-success-foreground rounded hover:bg-success/90">
                            收货
                          </button>
                        )}
                        {isPurchasing && po.status === 'rejected' && (
                          <button onClick={() => updatePOStatus(po.poNumber, 'draft')} className="px-2 py-1 text-xs border rounded hover:bg-secondary">
                            重新编辑
                          </button>
                        )}
                      </div>
