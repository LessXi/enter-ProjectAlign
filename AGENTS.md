...
| 文件 | 说明 |
|------|------|
| `src/hooks/useAuth.tsx` | React Context 认证。提供 `user`/`session`/`role`/`displayName`/`signIn`/`signUp`/`signOut`。支持姓名登录（通过 RPC `lookup_email_by_name`） |
...
1. **现金流方向**：入库 = 花钱（成本/支出），出库 = 赚钱（收入）。净现金流 = 出库金额 - 入库金额。
...
7. **认证**：支持邮箱和姓名登录。注册限仓管/采购角色，老板账号由管理员创建。
