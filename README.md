...
### 工作台 (Dashboard)
- Bento Grid 响应式布局（手机单列、平板双列、桌面四列）
- KPI 卡片（SKU 数、预警商品、入库额、出库额、库存总值、待审批）
- 本月进出趋势组合图：按周汇总金额柱状图 + 利润折线（右侧 Y 轴）
- 库存预警表（采购角色可一键跳转补货）
...
src/
  ...
  components/
    ...
    ui/                    — shadcn/ui 基础组件
  hooks/
    useAuth.tsx            — 认证 Context（登录/登出/角色）
